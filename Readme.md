...
A Python script to download Sri Lankan songs in **Real Audio (.ra/.ram)** format from [InfoLanka Miyuru Gee](http://www.infolanka.com/miyuru_gee/). It automatically processes `.ram` files, renames them using metadata, and cleans up temporary files.
...
> [!TIP]
> You can adjust `MAX_WORKERS` within `music.py` to optimize download speeds for your connection.
